        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="relative min-h-[300px]">
            {loading ? (
              ...
              <div className="bg-red-900/30 text-red-300 p-4 rounded-xl text-center">
                {error}
              </div>
            ) : selectedLevel ? (
              <>
                ...
                {scholarships.length === 0 ? (
                  <div className="text-center py-8 bg-zinc-800/50 rounded-xl">
                    ...
                    <p className="text-gray-300">No scholarships available for this level.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {scholarships.map((scholarship, index) => (
                      <Card3D key={index} className="w-full">
                        <Card
                          className={`border-none bg-zinc-800/80 backdrop-blur-sm shadow-xl animate-slideInBottom delay-${
                            index * 100
                          }`}
                        >
                          <CardContent className="p-6">
                            <div className="flex justify-between items-start mb-4">
                              <h3 className="text-xl font-semibold text-white">{scholarship["Scholarship Name"]}</h3>
                            ...
                            <div className="flex flex-wrap gap-4 mb-4">
                              {scholarship.Amount && (
                                <div className="bg-zinc-700/50 px-3 py-1 rounded-full text-sm text-gray-200">
              ...
              <div className="text-center py-16 bg-zinc-800/50 rounded-xl">
                <GraduationCap className="h-16 w-16 text-pink-500 mx-auto mb-4 animate-float" />
                <h3 className="text-xl font-semibold mb-2 text-white">Select an Education Level</h3>
          ...
          <div className="mt-16 bg-zinc-800/50 rounded-xl p-8">
            <h2 className="text-2xl font-semibold mb-4 text-center bg-gradient-to-r from-pink-500 to-purple-600 text-transparent bg-clip-text">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
              {[
                {
                  title: "Prepare Early",
                  description: "Start gathering documents and preparing your application well before the deadline.",
                  icon: <BookOpen className="h-6 w-6 text-pink-500" />,
                },
                {
                  title: "Read Requirements Carefully",
                  description: "Ensure you meet all eligibility criteria before applying to increase your chances.",
                  icon: <GraduationCap className="h-6 w-6 text-pink-500" />,
                },
                {
                  title: "Submit Quality Essays",
                  description: "Take time to craft compelling essays that showcase your achievements and aspirations.",
                  icon: <Award className="h-6 w-6 text-pink-500" />,
                },
              ].map((tip, index) => (
                <div
                  key={index}
                  className="bg-zinc-700/50 p-6 rounded-xl hover-lift animate-fadeIn"
                  style={{ animationDelay: `${index * 200}ms` }}
                >
                  <div className="p-3 rounded-full bg-pink-500/20 w-fit mb-4">{tip.icon}</div>
                  <h3 className="text-lg font-semibold mb-2 text-white">{tip.title}</h3>
